...
use vectors_db::api::rbac::{ApiKeyEntry, RbacConfig, Role};
use vectors_db::storage::{Database, WriteAheadLog};
...
// Helper: create a collection with given name and dimension
...
// Helper: insert a document and return the response
...
// Helper: insert a document with metadata
...
async fn test_filter_gt_integer() {
    ...
    let resp = client()
        ...
        .json(&serde_json::json!({
            ...
            "k": 10,
            "filter": {
                "must": [{"field": "score", "op": "gt", "value": 50}]
            }
        }))
        .send()
        .await
...
async fn test_filter_in_operator() {
    ...
    let resp = client()
        ...
        .json(&serde_json::json!({
            ...
            "k": 10,
            "filter": {
                "must": [{"field": "animal", "op": "in", "values": ["cat", "dog"]}]
            }
        }))
        .send()
        .await
...
async fn test_filter_must_not() {
    ...
    let resp = client()
        ...
        .json(&serde_json::json!({
            ...
            "k": 10,
            "filter": {
                "must_not": [{"field": "status", "op": "eq", "value": "archived"}]
            }
        }))
        .send()
        .await
...
async fn test_filter_no_match() {
    ...
    let resp = client()
        ...
        .json(&serde_json::json!({
            ...
            "k": 10,
            "filter": {
                "must": [{"field": "type", "op": "eq", "value": "nonexistent"}]
            }
        }))
        .send()
        .await
...
async fn test_update_dimension_mismatch() {
    ...
    // Update with wrong dimension embedding
    let resp = client()
        .put(format!(
...
// ========== Feature 5: Collection Stats & Memory Limits ==========
...
async fn test_rebuild_index() {
    ...
    // Verify stats show 0 deleted after rebuild
    let resp = client()
...
async fn test_rbac_read_only() {
    ...
    // Read-only key can list collections
    let resp = client()
        .get(format!("{}/collections", base_url))
    ...
    // Read-only key cannot create collection
    let resp = client()
        .post(format!("{}/collections", base_url))
        .header("Authorization", "Bearer read-key")
...
async fn test_rbac_write_role() {
    ...
    // Write key can create collection
    let resp = client()
        .post(format!("{}/collections", base_url))
...
async fn test_rbac_backward_compat() {
    // When using legacy single API key, all requests work with it
...
// ========== Phase 5: Request Size Limits ==========

#[tokio::test]
async fn test_request_body_too_large() {
    let (base_url, _tmp) = spawn_app_no_recorder(None).await;

    // Send a payload larger than MAX_REQUEST_BODY_BYTES (10 MB)
...
async fn test_prefilter_selective() {
    // Insert 50 documents, only 2 with category="target". Pre-filtering should find both.
    let (base_url, _tmp) = spawn_app_no_recorder(None).await;
    create_test_collection(&base_url, "prefilter", 3).await;

    // Insert 20 non-matching documents with diverse embeddings
    for i in 0..20 {
    ...
    // Search with filter — should find both target documents
    let resp = client()
        ...
        .json(&serde_json::json!({
            ...
            "k": 10,
            "filter": {
                "must": [{"field": "category", "op": "eq", "value": "target"}]
            }
        }))
        .send()
        .await
    ...
    // Verify all results have the target category
    for r in results {
        assert_eq!(r["metadata"]["category"], "target");
    }
}

// ========== Phase 5: Standalone mode (no Raft) ==========

#[tokio::test]
async fn test_standalone_mode_works() {
    // Without --node-id, server works exactly like before (raft=None)
    let (base_url, _tmp) = spawn_app_no_recorder(None).await;
...
async fn test_routing_table_with_local_assignment() {
    // In standalone mode, assign a collection and verify routing table
    ...
    let data_dir = tmp_dir.path().to_str().unwrap().to_string();
    let db = vectors_db::storage::Database::new();
    let wal = Arc::new(vectors_db::storage::WriteAheadLog::new(&data_dir).unwrap());
    ...
    let app = vectors_db::api::create_router(state);
    let listener = tokio::net::TcpListener::bind("127.0.0.1:0").await.unwrap();
    let addr = listener.local_addr().unwrap();
    let base_url = format!("http://{}", addr);
    tokio::spawn(async move { axum::serve(listener, app).await.unwrap() });

    // Assign a collection to this node
    let resp = client()
...
async fn test_metadata_too_many_keys() {
    ...
    // Build metadata with 65 keys (limit is 64)
...
async fn test_metadata_too_large() {
    ...
    // Build metadata with a single very large value (> 64 KB)
...
async fn test_error_messages_sanitized() {
    ...
    // Try to load a non-existent collection — should not leak file paths
    let resp = client()
    ...
    // Should NOT contain internal details like file paths or "No such file"
    assert!(
