...
fn main() {
    println!("=== ANN Benchmark: SIFT-128 euclidean (L2) ===");
    println!();

    print!("Loading train vectors...");
    let (train_count, dim, train_vectors) = read_fvecs(&format!("{DATA_DIR}/sift128_train.bin"));
    println!(" {train_count} vectors x {dim}d");

    print!("Loading test queries...");
    let (test_count, _, test_vectors) = read_fvecs(&format!("{DATA_DIR}/sift128_test.bin"));
    println!(" {test_count} queries");

    print!("Loading ground truth...");
    let (_, gt_k, ground_truth) = read_ground_truth(&format!("{DATA_DIR}/sift128_neighbors.bin"));
    ...
    let config = HnswConfig {
        ...
        ef_search: 50,
        max_layers: 16,
        distance_metric: vectors_db::hnsw::DistanceMetric::Euclidean,
    };

    println!("Config: M=16, ef_c=200, metric=Euclidean");
    ...
    println!();
    println!("  --- Reference (ann-benchmarks.com, SIFT-128 euclidean, k=10) ---");
    println!("  Algorithm          | Recall@10 |      QPS | Notes");
    println!("  -------------------+-----------+----------+------");
    println!("  hnsw(nmslib)       |    0.9989 |    4,288 | C++ HNSW");
    println!("  hnswlib            |    0.9941 |    1,244 | C++");
    println!("  scann (Google)     |    0.9990 |   13,040 | Quantization + HNSW");
    println!("  hnsw(faiss)        |    0.9992 |      652 | Facebook");
    println!("  Annoy (Spotify)    |    0.9900 |      502 | Tree-based");
    println!("  Vamana(diskann)    |    0.9999 |    5,765 | Microsoft DiskANN");
    println!("  glass              |    0.9999 |    4,007 | Graph-based");
    println!("  pgvector           |    0.9890 |       16 | PostgreSQL");
    println!();
